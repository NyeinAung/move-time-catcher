...
class ToDoListEdit extends Component{
    ...
    // get todo lists detail
    getDetail() {
        axios.get('/todo-lists/detail/'+this.state.id).then((response) => {
            if(response.data.status === 200){
                if(response.data.data) {
                    this.setState({alarm_hour: response.data.data.alarm[0], 
                                alarm_mins: response.data.data.alarm[1],
                                
                                time_to_teeth_hour: response.data.data.time_to_teeth[0],
                                time_to_teeth_mins: response.data.data.time_to_teeth[1],

                                breakfast_time_hour: response.data.data.breakfast_time[0],
                                breakfast_time_mins: response.data.data.breakfast_time[1],
    ...
    handleSubmit(e) {
        ...
        if(this.handleValidation()){
            ...
            axios.post(`/todo-lists/update/${this.state.id}`, {
                alarm_hour: this.state.alarm_hour,
                alarm_mins: this.state.alarm_mins,

                time_to_teeth_hour: this.state.time_to_teeth_hour,
                time_to_teeth_mins: this.state.time_to_teeth_mins,

                breakfast_time_hour: this.state.breakfast_time_hour,
                breakfast_time_mins: this.state.breakfast_time_mins,
            }).then((response) => {
                if(response.data.status === 200){
